...
<template>
  <div class="auth-page">
    <div class="auth-card">
      ...
      <h1>注册 Together</h1>
      <p class="subtitle">加入 Together，体验无缝的文档协作。</p>
      <form @submit.prevent="submit">
        <div class="input-group">
          <input v-model="form.username" placeholder="用户名" required />
        </div>
        <div class="input-group">
          <input v-model="form.displayName" placeholder="昵称" required />
        </div>
        <div class="input-group">
          <input v-model="form.email" placeholder="邮箱地址" type="email" required />
        </div>
        <div class="input-group">
          <input v-model="form.password" placeholder="密码（至少 6 位）" type="password" required />
        </div>
...
const error = ref('')
const form = reactive({ username: '', displayName: '', email: '', password: '' })
